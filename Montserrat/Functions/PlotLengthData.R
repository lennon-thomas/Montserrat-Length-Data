PlotLengthData<- function(LengthDat,FigureFolder,Fish,Species,Site,Theme)
  ...
  pdf(file=paste(FigureFolder,Species,'-',Site,'Length Data.pdf',sep=''),width=6,height=4)
  
  LengthDist<-(ggplot(data=LengthDat,aes(Length,fill=(SiteType)))+geom_density()
  +scale_fill_manual(name='',values=c(FishedColor,MPAColor))
  +geom_vline(xintercept=Fish$Mat50,linetype='longdash')+
    geom_vline(xintercept=Fish$Linf,linetype='longdash',color='red2')+scale_y_continuous(expand = c(0, 0))+scale_x_continuous(lim=c(0,25))+
  ggtitle(paste(Species,sep='\nn = ',length(LengthDat$Length)))+Theme+xlab("Length (cm)")+ylab("Density"))
  LengthDist<-LengthDist+theme(legend.position="none")
  print(LengthDist)
