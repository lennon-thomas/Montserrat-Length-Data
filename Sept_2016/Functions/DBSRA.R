...
#   Version 4d
#   CatchDat<- CatchData
#   DCAC.start.yr <- 1994 #start of the catch period
#   DCAC.end.yr<- 2014 #end of the catch period
#   delta.yr<- 2014 #Year that current depletion is fit to
#   DBSRA.OFL.yr<- 2014 #Year to calculate DBSRA OFL outputs
#   M.est<- .111 #Natural morality estiamte
...
#   BMSYtoB0LowerBound<- 0.1
#   BMSYtoB0UpperBound<- 1.1
#  
  N.spp<-1
   InterpCatch<-1
  ...
  for (i in 1:N.spp)
  {
    ...
    set.seed(myseed[1])
    M.vec     <- rlnorm(NumIterations, meanlog=(log(M)-0.5*SD.lnM^2), sdlog=SD.lnM)
    
    # lognormal distribution for ratio of ratio FMSY/M
    set.seed(myseed[2])
    FMSYtoM.vec  <- rlnorm(NumIterations, meanlog=(log(FMSYtoMratio)-0.5*SD.FMSYtoMratio^2), sdlog=SD.FMSYtoMratio)
    
    # simulate bounded beta draws for delta
    set.seed(myseed[3])
    Delta.vec <- rbeta.ab(NumIterations, Delta, SD.Delta,
                          DeltaLowerBound, DeltaUpperBound)
    
    # simulate draws for ratio of BMSY/B0
    set.seed(myseed[4])
    BMSYtoB0.vec  <- rbeta.ab(NumIterations, BMSYtoB0ratio, SD.BMSYtoB0ratio,
  ...
  for (i in 1:N.spp)
  {
    ...
    x.tmp[i,2:6] <- quantile(B0.list[[i]], probs=c(0.025, 0.25, 0.5, 0.75, 0.975),na.rm=TRUE)
  }
  ...
  for (y in 1:length(yr))
  {
    ...
    Quants<- as.data.frame(apply(YearData,2,quantile,probs=c(0.025, 0.25, 0.5, 0.75, 0.975),na.rm=TRUE))
  ...
  title(xlab='Year',ylab='Biomass (kg)')
  dev.off()
  ...
  par(mfrow=c(3,1))
  plot(OFLStats$OFL~OFLStats$Year,type='l',bty='n',xlab='Year',ylab='OFL (kg)')
  ...
  plot(BioStats$Biomass ~ BioStats $Year,type='l',bty='n',xlab='Year',ylab='Biomass (kg)')
  ...
  for (i in 1:N.spp)
  {
    ...
    x.tmp[i,2:6] <- quantile(OFL.list[[i]][,OFL.yrs[i]], probs=c(0.025, 0.25, 0.5, 0.75, 0.975),na.rm=TRUE)
  }
