PlotLengthData<- function(LengthDat,FigureFolder,Fish,Species,Theme)
{
  ...
  FLA<-nrow(LengthDat[LengthDat$Data.Type=="Fishery",])
  SLA<-nrow(LengthDat[LengthDat$Data.Type=="Survey",])
 ...
 pdf(file=paste(FigureFolder,Species,'_Length Hist.pdf',sep=''),width=7,height=5)
  LengthDist<-ggplot(data=LengthDat,aes(x=Length,fill=Data.Type,color=Data.Type))+geom_histogram(position="identity",binwidth=1,alpha=0.6)+
  geom_vline(xintercept=Fish$Mat50,linetype='longdash')+
   ...
   ggtitle(paste(Species))+Theme+xlab("Length (cm)")+ylab("Frequency")+
