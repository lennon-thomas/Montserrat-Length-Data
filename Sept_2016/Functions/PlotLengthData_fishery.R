PlotLengthData<- function(LengthDat,FigureFolder,Fish,Species,Theme)
{
  ...
  pdf(file=paste(FigureFolder,Species,'_Length Hist_fishery.pdf',sep=''),width=7,height=5)
  LengthDist<-ggplot(data=LengthDat,aes(x=Length))+
    geom_histogram(position="identity",binwidth=1,alpha=0.6,fill="red",col="black")+
